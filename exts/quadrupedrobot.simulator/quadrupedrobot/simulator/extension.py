"""
| File: extension.py
| Author: Jinwon Kim (mqjinwon@gmail.com)
| License: BSD-3-Clause. Copyright (c) 2023, Jinwon Kim. All rights reserved.
| Description: Implements the QuadrupedRobot_Extension which omni.ext.IExt that is created when this class is enabled. In turn, this class initializes the extension widget.
"""
__all__ = ["QuadrupedRobot_Extension"]
...
from quadrupedrobot.simulator.params import MENU_PATH, WINDOW_TITLE
from quadrupedrobot.simulator.logic.interface.pegasus_interface import PegasusInterface

# Setting up the UI for the extension's Widget
from quadrupedrobot.simulator.ui.ui_window import WidgetWindow
from quadrupedrobot.simulator.ui.ui_delegate import UIDelegate


# Any class derived from `omni.ext.IExt` in top level module (defined in `python.modules` of `extension.toml`) will be
# instantiated when extension gets enabled and `on_startup(ext_id)` will be called. Later when extension gets disabled
# on_shutdown() is called.
class QuadrupedRobot_Extension(omni.ext.IExt):
    ...
    def autoload_helper(self):
        ...
        if (
            ...
            and type(get_active_viewport().stage) == pxr.Usd.Stage
            and str(get_active_viewport().stage.GetPrimAtPath("/OmniverseKit_Persp"))
            != "invalid null prim"
        ):
            self._pegasus_sim.initialize_world()
        else:
